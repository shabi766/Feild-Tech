...
const connectDB = async () => {
    try {
        ...
        const mongoURI = process.env.WORKORDER_DB_URI || process.env.MONGO_URI || "mongodb://localhost:27017/fieldtech_workorder";
        await mongoose.connect(mongoURI);
        console.log('✅ Workorder Service: MongoDB connected successfully');
    } catch (error) {
        console.log('❌ Workorder Service: MongoDB connection failed:', error.message);
