...
const connectDB = async () => {
    try {
        ...
        const mongoURI = process.env.ADMIN_DB_URI || process.env.MONGO_URI || "mongodb://localhost:27017/fieldtech_admin";
        await mongoose.connect(mongoURI);
        console.log('✅ Admin Service: MongoDB connected successfully');
    } catch (error) {
        console.log('❌ Admin Service: MongoDB connection failed:', error.message);
