 ...
 * Use this in other services to fetch client and project data
 */
...
export class ClientServiceClient {
     ...
     */
    static async getProjectsByClientId(clientId, token) {
        try {
            const response = await fetch(`${CLIENT_SERVICE_URL}/api/v1/project/get-by-client/${clientId}`, {
                headers: { 
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
            ...
            console.error('Error fetching projects from Client Service:', error);
            throw error;
        }
    }
}
