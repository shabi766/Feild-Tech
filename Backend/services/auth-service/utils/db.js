...
const connectDB = async () => {
    try {
        ...
        const mongoURI = process.env.AUTH_DB_URI || process.env.MONGO_URI || "mongodb://localhost:27017/fieldtech_auth";
        await mongoose.connect(mongoURI);
        console.log('✅ Auth Service: MongoDB connected successfully');
    } catch (error) {
        console.log('❌ Auth Service: MongoDB connection failed:', error.message);
