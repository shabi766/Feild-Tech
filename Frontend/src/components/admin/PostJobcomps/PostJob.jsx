...
const Shipments = React.lazy(() => import('./Shipments')); // Import the Shipments component
...
const PostJobs = () => {
  ...
  const [shipmentsData, setShipmentsData] = useState([]); // State to hold shipment data
  ...
  const saveAsDraft = async () => {
    const jobData = {
      ...
      tasks: tasks,
      shipments: shipmentsData, // Save shipment data as well
    };
  ...
  }, [input, customFields, tasks, shipmentsData]); // Include shipmentsData in the dependency array
        ...
        try {
          ...
          if (res.data.success && res.data.job) {
            if (res.data.job) {
              ...
              setTasks(job.tasks || []);
              setShipmentsData(job.shipments || []); // Load shipment data
              setStatus('Draft');
                    ...
                    <div className={styles.section}>
                      <section id="shipments" className="mb-4">
                        ...
                        <React.Suspense fallback={<div>Loading Shipments...</div>}>
                          <Shipments /> {/* Render the Shipments component here */}
                        </React.Suspense>
                      </section>
                    </div>
