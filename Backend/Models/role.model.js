...
const permissionSchema = new mongoose.Schema({
    ...
    // New Financial Management Permissions
...
const roleSchema = new mongoose.Schema({
    ...
    roleType: {
        type: String,
        enum: ['ADMIN', 'MANAGER', 'RECRUITER', 'COMPANY_OWNER', 'CUSTOM'], // Added COMPANY_OWNER for company owners
        default: 'CUSTOM'
    },
    isSystemRole: {
        type: Boolean,
...
// Pre-save middleware to ensure system roles have unique names across companies
roleSchema.pre('save', function(next) {
    if (this.isSystemRole) {
        this.companyId = null; // System roles are global
    }
    next();
});
